...
public class Picture extends Rectangle {
    ...
    public void paint(Graphics graphics, Scene scene, Dimension surfaceSize,
        ...
        Stroke selectedStroke
                = (config.selectedShapeBorderWidth > 0
                   && config.selectedShapeBorderColor != null
                   ? new BasicStroke(config.selectedShapeBorderWidth)
                   : null);
        ...
        if (config.reflectionOpacity > 0) {
            ...
            if (selected) {
                if (config.selectedShapeOverlayColor != null) {
                    g.setColor(config.selectedShapeOverlayColor);
                    g.fillPolygon(xPoints, ryPoints, 4);
                }
                // FIXME outer border doesn't receive overlay, so disable this
                // if (activeStroke != null) {
        ...
        if (selected) {
            ...
            if (selectedStroke != null) {
                g.setColor(config.selectedShapeBorderColor);
